...
describe('GET /booking', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 404 when user do not payed ticket yet', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
    ...
    it('should respond with status 404 when ticket does not includes hotel', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
...
describe('POST /booking', () => {
  ...
  describe('when token is valid', () => {
    it('should respond with status 400 when user does not send a  body', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
    ...
    it('should respond with status 403 when user does not has a ticket', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      await createEnrollmentWithAddress(user);
    ...
    it('should respond with status 403 when user does not has a enrollment yet', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const response = await server.post('/booking').set('Authorization', `Bearer ${token}`).send({
        roomId: 1,
    ...
    it('should respond with status 403 when user do not payed ticket yet', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType();
    ...
    it('should respond with status 200 when  is permited to reserve', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType(false, true);
